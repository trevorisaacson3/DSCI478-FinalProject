...
```{r}
snowTotals = stack(ParkCity2019[2:ncol(ParkCity2019)])
snowTotals = snowTotals[-c(60,61,62,124,168,186,248,341), ]
newParkCity2019 = data.frame(Year = DailyData$Year,
                             Month = DailyData$Month,
                             Day = DailyData$Day,
                             Snowfall = snowTotals$values)
...
ggplot(data = ParkCity2019, aes(x = Day, y = Snowfall)) +
  geom_line() +
  facet_wrap(~Month)
```
