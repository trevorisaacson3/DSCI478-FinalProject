...
Exploratory Factor Analysis Function For Actual Prediction
```{r}
factorAnalysis = function(df){
  X = df[,1:11]
  n <- length(df$AvgSnowfall3DayWindow)
  ...
  for(i in c(1,2,3)) {
    for(j in c("varimax", "none")) {
      ...
      #MLE
      data <- cbind(df$AvgSnowfall3DayWindow, f$scores)
      ...
      rmseVals[count] <- rmse(testing$V1, pred)
      count = count + 1;
      
      #Principal
      data <- cbind(df$AvgSnowfall3DayWindow, p$scores)
      data = as.data.frame(data) 
      training2 <- data[trn,]
      testing2 <- data[-trn,]
      ...
      rmseVals[count] <- rmse(testing2$V1, pred2)
      count = count + 1;
    }
  }
  print(rmseVals)
...
```{r  warning = FALSE}
factorAnalysis(totalDailyJH)
factorAnalysis(totalDailyPC)
```
...
Bagging for Park City: 
```{r}
bag_fitPC <- randomForest(AvgSnowfall3DayBinary ~ ., data = trainingPC, mtry = ncol(trainingPC) - 1, importance = TRUE)
bag_predictPC = predict(bag_fitPC, testingPC, type = "response")
table(bag_predictPC, testingPC$AvgSnowfall3DayBinary)
print(paste("Test Error Rate of Class RF: ",1-((106+14)/(8+7+106+14))))
```


Bagging for Jackson Hole: 
```{r}
bag_fitJH <- randomForest(AvgSnowfall3DayBinary ~ ., data = trainingJH, mtry = ncol(trainingJH) - 1, importance = TRUE)
bag_predictJH = predict(bag_fitJH, testingJH, type = "response")
table(bag_predictJH, testingJH$AvgSnowfall3DayBinary)
print(paste("Test Error Rate of Class RF: ",1-((82+23)/(82+23+13+17))))
```

Exploratory Factor Analysis Function For Binary Predictions
```{r}
factorAnalysis = function(df){
  X = df[,1:11]
  n <- length(df$AvgSnowfall3DayBinary)
  trn <- sample(seq_len(n), .75*n)
  count = 1;
  rmseVals <- c()
  for(i in c(1,2,3)) {
    for(j in c("varimax", "none")) {
      ...
      #MLE
      data <- cbind(df$AvgSnowfall3DayBinary, f$scores)
      data = as.data.frame(data)
      training <- data[trn,]
      testing <- data[-trn,]
      model <- lm(V1 ~ ., data = training)
      ...
      #Principal
      data <- cbind(df$AvgSnowfall3DayBinary, p$scores)
      data = as.data.frame(data) 
      training2 <- data[trn,]
      testing2 <- data[-trn,]
      model2 <- lm(V1 ~ ., data = training2)
