...
tillamook2019 = tillamook2019[-c(137),]
...
advanceSnowTotals8Days = FoCoSnowTotals2019[-c(1:8), ]
advanceSnowTotals9Days = FoCoSnowTotals2019[-c(1:9), ]
...
SnowTotals8Daysinto2020 = FC2020[1:8, "Jan"]
SnowTotals9Daysinto2020 = FC2020[1:9, "Jan"]
...
Create Binary Snow Totals based on Day Range
```{r}
westCaliBreck = westCaliBreck %>%
...
capeFoCo = capeFoCo %>%
  mutate(AvgSnowfall3DayBinary = ifelse(AvgSnowfall3DayWindow > 0.0, 1, 0))  %>%
  mutate(AvgSnowfall3DayBinary = as.factor(AvgSnowfall3DayBinary))
tillamookBreck = tillamookBreck %>%
  mutate(AvgSnowfall3DayBinary = ifelse(AvgSnowfall3DayWindow > 0.0, 1, 0))  %>%
  mutate(AvgSnowfall3DayBinary = as.factor(AvgSnowfall3DayBinary))
tillamookFoCo = tillamookFoCo %>%
  mutate(AvgSnowfall3DayBinary = ifelse(AvgSnowfall3DayWindow > 0.0, 1, 0))  %>%
  mutate(AvgSnowfall3DayBinary = as.factor(AvgSnowfall3DayBinary))
```
...
The names are location of buoy and the snowfall totals for that location
```{r}
wcB = factorAnalysis(westCaliBreck)
wcF = factorAnalysis(westCaliFoCo)
cB = factorAnalysis(capeBreck)
cF = factorAnalysis(capeFoCo)
# tB = factorAnalysis(tillamookBreck)
```
...
cB
cF
Buoy = c("West Cali", "West Cali", "West Cali", "West Cali", "Cape Edgecumber", "Cape Edgecumber", "Cape Edgecumber", "Cape Edgecumber")
Resort = c("Breck", "Breck", "FoCo", "FoCo", "Breck", "Breck", "FoCo", "FoCo")
method = c("Fa", "Principal", "Fa", "Principal", "Principal", "Fa", "Principal", "Principal")
rotation = c("None", "None", "None", "Verimax", "None", "Verimax", "Verimax", "None")
factors = c("3", "3", "2", "1", "3", "3", "1", "2")
RMSEnumbers = c("0.4621", "0.4642", "0.4031", "0.4049", "0.4075", "0.4092", "0.3411", "0.3508")
finalFA = data.frame(Buoy = Buoy, Resort = Resort, Rotation = rotation, Factors = factors, RMSE_Inches = RMSEnumbers)
finalFA
```
