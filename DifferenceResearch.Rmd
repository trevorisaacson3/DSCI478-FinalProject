...
Add Jackson Hole data
Try removing summer months
...
originalBuoys["AvgSnowfall3DayWindow"] = ((originalBuoys$Snowfall13DaysLater + originalBuoys$Snowfall14DaysLater + originalBuoys$Snowfall15DaysLater) /3) /24
```


```{r}
timeLessData = originalBuoys %>%
  select(-c(Year, Day, Hour, Month, Snowfall13DaysLater, Snowfall14DaysLater, Snowfall15DaysLater))
predictedSnow = lm(AvgSnowfall3DayWindow ~ ., data = timeLessData)
summary(predictedSnow)
```

Trees
```{r}
initial_tree = tree(AvgSnowfall3DayWindow  ~ ., data = timeLessData)
summary(initial_tree)
plot(initial_tree)
text(initial_tree, cex = 0.65, digits = 4, pretty = 0)
initial_tree

# tree.cv = cv.tree(initial_tree)
# plot(tree.cv$size, sqrt(tree.cv$dev / nrow(initial)), type = "b", xlab = "Tree Size", ylab = "CV_RMSE")
...
Bagging: 
```{r}
bag_fit <- randomForest(AvgSnowfall3DayWindow ~ ., data = timeLessData, mtry = ncol(timeLessData) - 1, importance = TRUE)
plot(bag_fit)
